...
export function CanvasToolbar({ onFit, hideAspectRatio }: { onFit?: () => void; hideAspectRatio?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between border-t border-(--color-border) bg-(--color-bg-secondary) px-3 py-2">
      ...
      <div className="flex items-center gap-1">
        ...
        <button onClick={onFit || resetView} className="border border-(--color-border) px-2 py-1 text-xs text-(--color-text-secondary) hover:text-(--color-text) hover:bg-(--color-bg-tertiary)">
          Fit
        </button>
      </div>
    </div>
